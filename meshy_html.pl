...
$names{unix} = 'meshy';
$names{description} = 'unanticipated knowledge discovery through statistical ranking of MeSH term pairs';
...
if($chem eq 'on'){ `perl /labs/bat/www/tools/cgi-bin/limitresources.pl --command "perl meshy.pl --chem --threshold 5 --mhfreqfile MH_freq_alpha --chfreqfile Chemical_freq_alpha --mtree mtrees.bin --query '$query' --max_doc 10000 --pwd $upload_dir" --log $log --seconds $max_time --memory $max_mem > $results_filename &`;
